...
:project_id: draft-gs-spring-boot-gorm
This guide walks you through the process of creating a Spring Boot application that uses GORM and Hibernate for persistence.
...
Note that the constraint block defines any validation rules. 

== Create a resource controller

Create a new controller for your Spring/Groovy application:
...
The above example defines two REST endpoints - The `addPerson` endpoint allows the addition of new people and the `greet` endpoint greets a person by their first name.

You can create a new person by submitting a `POST` request. For example using the *nix `curl` tool:
...
Results in the following response indicating that a resource was created:
